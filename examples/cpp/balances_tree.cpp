...
typename hashes::sha2<256>::block_type hash_layer(std::array<typename hashes::sha2<256>::block_type, LayerSize> input) {
    ...
    for (std::size_t leaf_index = 0; leaf_index < LayerSize / 2; leaf_index++) {
        next_layer[leaf_index] = hash<hashes::sha2<256>>(input[2 * leaf_index], input[2 * leaf_index + 1]);
    }
    if (LayerSize == 2)
        return next_layer[0];
    else
...
[[circuit]] bool balance_tree([[private_input]] std::array<int64_t, precomputed_powers_of_two[validators_amount_log2]>
                                  validator_balances,
    ...
    constexpr std::size_t potentially_non_zero_leaves_amount =
        precomputed_powers_of_two[potentially_non_zero_leaves_amount_log2];
    ...
    typename hashes::sha2<256>::block_type non_zero_balances_subtree_root =
        hash_layer<potentially_non_zero_leaves_amount>(potentially_non_zero_leaves);

    constexpr std::size_t leaves_total_amount_log2 = precomputed_powers_of_two.size() - 1;
    constexpr std::size_t layer_with_zero_balance_subtrees_roots_size_log2 =
        leaves_total_amount_log2 - potentially_non_zero_leaves_amount_log2;
    constexpr std::size_t layer_with_zero_balance_subtrees_roots_size =
        precomputed_powers_of_two[layer_with_zero_balance_subtrees_roots_size_log2];

    std::array<typename hashes::sha2<256>::block_type, layer_with_zero_balance_subtrees_roots_size>
        layer_with_zero_balance_subtrees_roots;
    ...
    typename hashes::sha2<256>::block_type root =
        hash_layer<layer_with_zero_balance_subtrees_roots_size>(layer_with_zero_balance_subtrees_roots);
