...
for base_name in "${!base_names[@]}"; do
    ...
    for variant in "$base_name"*".ll"; do
        if [[ -f "$variant" ]]; then
            ...
            suffix=${suffix:-""}
            cp "$variant" "$target_dir/$base_name/bytecode${suffix}.ll"
        fi
    done

    if [[ -f "circuit_${base_name}.crct" ]]; then
        cp "circuit_${base_name}.crct" "$target_dir/$base_name/circuit.crct"
    fi
    if [[ -f "assignment_${base_name}.tbl" ]]; then
        cp "assignment_${base_name}.tbl" "$target_dir/$base_name/assignment.tbl"
    fi
done
